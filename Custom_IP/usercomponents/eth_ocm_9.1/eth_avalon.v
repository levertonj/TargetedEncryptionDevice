...
// Parameter RX_FIFO_DEPTH is the Depth of the Receive FIFO (in words)
// Parameter TX_FIFO_DEPTH is the Depth of the Transmit FIFO (in words)
module eth_avalon   #(  parameter   DESC_COUNT      = 128,
                        parameter   RX_FIFO_DEPTH   = 1024,
                        parameter   TX_FIFO_DEPTH   = 128,
                        parameter   MEM_WIDTH       = 32,
                        parameter   TX_BURST_ENABLE = 1,
                        parameter   TX_BURST_TARGET = 8,
                        parameter   RX_BURST_ENABLE = 1,
                        parameter   RX_BURST_TARGET = 8,
                        // NOT TO BE OVERRIDDEN
                        parameter   TX_BURST_CBITS  = clogb2(TX_BURST_TARGET),
                        parameter   RX_BURST_CBITS  = clogb2(RX_BURST_TARGET),
                        parameter   MBYTE_CNT       = MEM_WIDTH/8 )
    (
    ...
    input       [MEM_WIDTH-1:0] av_tx_readdata,     // Avalon TX master readdata
      //output
    output      [31:0]          av_tx_address,      // Avalon TX master address
    output      [TX_BURST_CBITS-1:0] av_tx_burstcount, // Avalon TX master burst count
    ...
    output      [MEM_WIDTH-1:0] av_rx_writedata,    // Avalon RX master writedata
    output      [MBYTE_CNT-1:0] av_rx_byteenable,   // Avalon RX master byteenable
    output      [RX_BURST_CBITS-1:0] av_rx_burstcount, // Avalon RX master burstcount
...
localparam  DESC_RAM_DEPTH  = RDC * 2;
...
wire    [7:0]   rx_bd_address;      // RX BD descriptor or pointer index
...
wire    [7:0]   tx_bd_address;      // TX BD descriptor or pointer index
wire    [31:0]  tx_bd_writedata;    // TX BD descriptor writeback data
//Muxed BD interface
wire    [31:0]  bd_readdata;        // Descriptor data from BD RAM 
wire            bd_write;           // Descriptor data write to BD RAM
wire    [7:0]   bd_address;         // Descriptor/Pointer index to BD RAM
...
assign  bd_address  = rx_txn_sel ? (rx_bd_address + {r_TxBDNum,1'b0}) : tx_bd_address;
...
// Descriptor BD RAM
eth_avalon_bd_ram #(.DEPTH(RDC*2) ) desc_ram(
    .clock      (av_clk         ),
...
//RX Interface
eth_avalon_rxdma    #(  .FIFO_DEPTH     (RX_FIFO_DEPTH),
                        .BURST_ENABLE   (RX_BURST_ENABLE),
                        .BURST_TARGET   (RX_BURST_TARGET),
                        .MEM_WIDTH      (MEM_WIDTH)
                        )   eth_rxdma_inst(
    ...
    .bd_rdata           (bd_readdata        ),  // Descriptor Control data input
    ...
    .bd_address         (rx_bd_address      ),  // Which descriptor to read
    .bd_wdata           (rx_bd_writedata    ),  // Control data to be written to descriptor
    ...
    .av_burstcount      (av_rx_burstcount   ),  // Memory port burstcount
...
//TX Interface
eth_avalon_txdma   #(   .FIFO_DEPTH     (TX_FIFO_DEPTH),
                        .BURST_ENABLE   (TX_BURST_ENABLE),
                        .BURST_TARGET   (TX_BURST_TARGET),
                        .MEM_WIDTH      (MEM_WIDTH)
                    ) eth_txdma_inst(
    //Common signals
    .clk                (av_clk             ),  // Avalon clock
    ...
    .bd_address         (tx_bd_address      ),  // Which descriptor to read
    .bd_wdata           (tx_bd_writedata    ),  // Control data to be written to descriptor
    ...
    .av_burstcount      (av_tx_burstcount   ),  // Memory port burstcount
